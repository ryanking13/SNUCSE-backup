...
#
if ($USE_BTEST) {
    print "\n4. Compiling and running './btest -g -r 2.5' to determine performance score.\n";
    system("cp Zap-bits.c bits.c");
    ...
    # Run btest
    $status = system("./btest -g -r 2.5 > btest-Zapped.out 2>&1");
    if ($status != 0) {
	die "$0: ERROR: Zapped btest failed. $diemsg\n";
    }
}
else {
    print "\n4. Running './bddcheck/check.pl -g -r 2.5' to determine performance score.\n";
    system("cp Zap-bits.c bits.c");
    $status = system("./bddcheck/check.pl -g -r 2.5 > btest-Zapped.out 2>&1");
    if ($status != 0) {
