...
int mm_check(void){
	...
	for(list = free_lists; list < free_lists + NUM_LISTS; list++){
		for(node = *list; node != NULL; node = (int*)*(node+1)){
			...
			else if(block_size == (size_t)0){
				printf("SIZE 0 BLOCK FOUND\n");
				printf("node: %p\n", node);
				return -1;
			}
...
void *mm_malloc(size_t size){
	...
	size_t block_size; // 8-byte aligned size + tags
	size_t old_block_size; // original block size
	...
	for (list = size_to_list(block_size); list < free_lists + NUM_LISTS; list++){
		...
		for(node = *list; node != NULL; node = (int*)*(node+1)){
			...
			// printf("old block size %d\n", old_block_size); // FOR DEBUG
			
			// removd old node from free list
			printf("malloc remove call\n");
			...
			*footer = block_size | ALLOCATED;
			...
			// if can use remain heap area
			if(old_block_size - block_size < 0){
				header = footer+1;
				block_size = old_block_size - block_size;
				if(block_size % 8 != 0){
					printf("ALIGN ERROR\n");
					exit(0);
				}
				footer = (void*)header + block_size - 4;
				*header = block_size;
				*footer = block_size;
				add_node(size_to_list(block_size), header+1);
			}
			return node;
		}
	}
...
void mm_free(void *ptr){
	...
	if(!(*(next_header) & ALLOCATED)){
		...
		printf("next call: %p %p %d\n", next_header, mem_heap_hi(), next_size);
		remove_node(size_to_list(next_size), (int*)(next_header+1));
...
int* allocate_new_node(size_t block_size){
	...
	// printf("block size, last size: %d %d\n", block_size, last_block_size);
	...
	*(int*)((void*)node + block_size - 4) = ALLOCATED;
	printf("boundary: %p %p\n", (int*)((void*)node + block_size - 4), mem_heap_hi());
