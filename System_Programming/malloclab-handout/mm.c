...
#define NUM_LISTS 30 // number of free lists
#define ALIGNMENT 8
...
void *mm_realloc(void *ptr, size_t size){
	...
	header = GET_HEADER(ptr);
	old_block_size = GET_SIZE(header);
	footer = GET_FOOTER(ptr, old_block_size);
	...
	if(old_block_size < block_size){
		...
		// if next block is free block and coalesing it can store realloc size
		if( (!(*next_header & ALLOCATED)) & (next_size + old_block_size >= block_size)){
			block_size = next_size + old_block_size;

			remove_node(size_to_list(next_size), (int*)(footer+2));
			footer = (int *)((void*)footer + next_size);
		...
		// just malloc new block
		newptr = mm_malloc(size);
		memcpy(newptr, ptr, old_block_size);
		mm_free(ptr);
		return newptr;
	}

	// size not changeing case
	else if(old_block_size <= block_size + 8){
		return ptr;
	}
	...
	// shrinking case
		...
		footer = GET_FOOTER(header+1, block_size);
